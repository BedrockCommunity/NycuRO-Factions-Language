...
public class MessageAPI {
    ...
    public void sendCooldownMessage(Player player, long timeGone) {
        int lang = Database.profile.get(player.getUniqueId()).getLanguage();
            switch (lang) {
                case 0:
                    player.sendMessage("§7(§3!§7) §4Error: §7It hasn't been 24 hours before you can use the kits! Has gone only §b" + Core.time(timeGone) + " §7!");
                    break;
                case 1:
                    player.sendMessage("§7(§3!§7) §4Error: §7Nu au trecut 24 ore pentru a putea folosi kit-urile! S-au dus doar §b" + Core.time(timeGone) + " §7!");
                    break;
            }
    }
    ...
    public void getPlayerTimeMessage(CommandSender commandSender, Player getter, long time, long totalTime) {
        if (commandSender instanceof Player) {
            ...
            switch (lang) {
                case 0:
                    commandSender.sendMessage("§7» §b" + getter.getName() + " §3was active on this session for §e" + Core.time(time) + " §7!");
                    commandSender.sendMessage("§7» §b" + getter.getName() + " §3was active on this server for §e" + Core.time(totalTime) + " §7!");
                    break;
                case 1:
                    commandSender.sendMessage("§7» §b" + getter.getName() + " §3a fost activ pe aceasta sesiune pentru §e" + Core.time(time) + " §7!");
                    commandSender.sendMessage("§7» §b" + getter.getName() + " §3a fost activ pe acest server pentru §e" + Core.time(totalTime) + " §7!");
                    break;
            }
        } else {
            commandSender.sendMessage("§7» §b" + getter.getName() + " §3a fost activ pe aceasta sesiune pentru §e" + Core.time(time) + " §7!");
    ...
    public void getSelfTimeMessage(Player player, long time, long totalTime) {
        ...
        switch (lang) {
            case 0:
                player.sendMessage("§7(§3!§7) §bYou was active on this session for §e" + Core.time(time) + " §7!");
                player.sendMessage("§7(§3!§7) §bYou was active on server for §e" + Core.time(totalTime) + " §7!");
                break;
            case 1:
                player.sendMessage("§7(§3!§7) §bAi fost activ pe aceasta sesiune timp de §e" + Core.time(time) + " §7!");
                player.sendMessage("§7(§3!§7) §bAi fost activ pe server timp de §e" + Core.time(totalTime) + " §7!");
                break;
        }
    }
